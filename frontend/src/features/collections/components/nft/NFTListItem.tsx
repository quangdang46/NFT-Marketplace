...
export default function NFTListItem({ nft }: NFTListItemProps) {
  ...
  const floorDiff = () => {
    ...
    if (diff > 0) return { value: `+${diff.toFixed(2)}%`, color: "text-green-500" }
    ...
    <div className="grid grid-cols-5 gap-4 items-center p-4">
      <div className="flex items-center gap-3">
        <Button
          ...
          size="icon"
          className={cn("h-6 w-6 rounded-full flex-shrink-0", nft.selected ? "bg-pink-600 hover:bg-pink-700" : "")}
        >
          {nft.selected && <Check className="h-3 w-3" />}
        </Button>

        <div className="flex items-center gap-3">
          <div className={cn("h-12 w-12 rounded overflow-hidden flex-shrink-0", getBgColor(nft.background))}>
            <Image
      ...
      <div>
        ...
        <div className="text-xs text-muted-foreground">(${(Number.parseFloat(nft.price) * 80000).toFixed(2)})</div>
      </div>
      ...
      <div className="text-muted-foreground">
        {Number.parseInt(nft.id) % 2 === 0 ? "about 7 hours ago" : "about 4 days ago"}
      </div>
    </div>
  )
}
