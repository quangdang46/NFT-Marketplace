...
contract NFTMarketplace is ERC721URIStorage, Ownable, IERC2981 {
    ...
    // Lazy Minting
    mapping(bytes => bool) private _usedSignatures; // Tránh tái sử dụng signature
    ...
    function mintNFT(string memory uri) external returns (uint256) {
        ...
        _setTokenURI(tokenId, uri);
        royaltyRecipients[tokenId] = msg.sender; // Gán creator
        royaltyPercentages[tokenId] = 1000; // Mặc định 10% royalty
        emit NFTMinted(tokenId, msg.sender, uri);
        return tokenId;
    }

    // Lazy Minting (Mint khi mua)
    function lazyMint(
        address buyer,
        string memory uri,
    ...
    function createAuction(uint256 tokenId, uint256 minBid, uint256 duration) external {
        ...
        _transfer(msg.sender, address(this), tokenId); // Chuyển NFT vào contract
    ...
    function placeBid(uint256 tokenId) external payable {
        ...
        if (auction.highestBidder != address(0)) {
            payable(auction.highestBidder).transfer(auction.highestBid); // Hoàn tiền bidder cũ
        }
    ...
    function endAuction(uint256 tokenId) external {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            _transfer(address(this), auction.seller, tokenId); // Hoàn NFT nếu không có bidder
        }
    }
