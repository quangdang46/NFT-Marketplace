...
import { ChainController } from '@/v1/chain.controller';
...
const SERVICE_NAME = 'chain-service';
...
const CONTROLLERS = [ChainController];
...
const PROVIDERS = [
  ...
  },
  {
    ...
    useFactory: async () => {
      ...
      const logger = new Logger('chainModule');
      logger.log('chain Service registered with Consul');
      return discovery;
    },
    inject: [],
  },
...
export class ChainModule implements OnModuleInit {
  private readonly logger = new Logger(ChainModule.name);

  constructor(private readonly rabbitMQHealthService: RabbitMQHealthService) {
    this.logger.log('chain Module initialized');
  }
  ...
  async onModuleInit() {
    ...
    this.logger.log('Starting health check for chainModule');
